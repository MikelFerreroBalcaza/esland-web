				...
				{candidatos?.map((candidate, index) => {
					...
					return (
						<li class={`relative rounded-lg`}>
							{enlace && (
								<a
									class='youtube-link text-xs peer:hover:opacity-0 transition w-6 h-6 flex justify-center items-center right-2 top-2 absolute bg-white hover:bg-sky-200 backdrop-blur-xl text-black z-10 rounded-full hover:scale-125'
									href={enlace}
									target='_blank'
									rel='noopener'>
									<CameraIcon />
	...
	return (
		<h1 class='relative [font-weight:100] m-auto mb-10 tracking-[1px] font-tomaso text-3xl max-w-xl text-center leading-snug flex justify-center items-center h-80 text-white'>
			...
			<div class='flex justify-center items-center flex-col gap-y-4 w-72 px-10'>
				<svg
					class='h-14 -translate-y-10 fill-current'
